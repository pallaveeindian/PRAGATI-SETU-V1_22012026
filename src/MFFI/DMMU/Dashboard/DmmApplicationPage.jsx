...
import DmmReviewModal from "./DmmReviewModal"; 
import DmmApplicationFilters from "./DmmApplicationFilters"; 

export default function DmmApplicationListPage() {

  // Active Tab State
  const [activeTab, setActiveTab] = useState("PENDING");

  // Complete Mock Data Array
  ...
  const handleForwardToBank = (app) => {
    ...
    closeModal();
    alert(`${app.memberName}'s application forwarded to Bank successfully!`);
  };
  ...
  const handleRevert = (app, reason) => {
    ...
    setFilteredApps(updatedApps);
    closeModal();
    alert(`Application ${app.id} Reverted back to BMMU successfully!`);
  };

  // 🟢 NAYA LOGIC: HTML TO EXCEL WITH STYLING AND HEADERS
  ...
  return (
    <div style={{ width: "100%", position: "relative" }}>
      ...
      {/* Header Layout with Export Button */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <h2 style={{ color: "#1F3C88", margin: 0 }}>DMMU Application Verification List</h2>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "14px" }}>
            ...
            <tbody>
              {displayedApps.map((app, index) => {
                ...
                const actionBtnText = (app.status === "PENDING" || app.status === "REVERTED_BY_BANK") 
                  ? "View & Verify" 
                  : "View";

                let statusLabel = "";
                if (app.status === "PENDING") statusLabel = "Pending at DMMU";
                if (app.status === "FORWARDED") statusLabel = "Sent to Bank";
                if (app.status === "REVERTED") statusLabel = "Reverted to BMMU";
                if (app.status === "REVERTED_BY_BANK") statusLabel = "Reverted by Bank";
                  ...
                  <tr key={app.id} style={{ borderBottom: "1px solid #eee", backgroundColor: "white" }}>
                    ...
                    <td style={styles.td}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "6px" }}>
                        ...
                        {(app.status === "REVERTED" || app.status === "REVERTED_BY_BANK") && (
                          <span style={{ color: app.status === "REVERTED_BY_BANK" ? "#b45309" : "#dc2626", fontSize: "11px", marginTop: "4px" }}>
...
const getStatusStyle = (status) => {
  ...
  if (status === "FORWARDED") return { ...baseStyle, backgroundColor: "#dcfce7", color: "#166534", border: "1px solid #bbf7d0" };
  if (status === "REVERTED") return { ...baseStyle, backgroundColor: "#fef2f2", color: "#b91c1c", border: "1px solid #fca5a5" };
  if (status === "REVERTED_BY_BANK") return { ...baseStyle, backgroundColor: "#fffbeb", color: "#b45309", border: "1px solid #fcd34d" };
...
const getDotStyle = (status) => {
  ...
  if (status === "FORWARDED") return { ...baseStyle, backgroundColor: "#16a34a" };
  if (status === "REVERTED") return { ...baseStyle, backgroundColor: "#dc2626" };
  if (status === "REVERTED_BY_BANK") return { ...baseStyle, backgroundColor: "#f59e0b" };
