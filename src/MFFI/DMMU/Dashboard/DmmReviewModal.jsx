...
export default function DmmReviewModal({ app, onClose, onForward, onRevert, isForwarded, isReverted, isRevertedByBank }) {
  const [bmmVerified, setBmmVerified] = useState(false);
  const [isReverting, setIsReverting] = useState(false);
  const [revertComment, setRevertComment] = useState("");

  // 🟢 Document Viewer ke liye naye States
    ...
    <div style={styles.modalOverlay}>
      <div style={styles.modalContent}>
        ...
        <div style={styles.modalHeader}>
          <div>
            <h3 style={{ margin: 0, color: "#1F3C88" }}>DMMU Application Review - {app.memberName}</h3>
        ...
        <div style={{ ...styles.sectionCard, backgroundColor: "#f8fafc", border: "1px solid #e2e8f0" }}>
          <h4 style={{ margin: "0 0 12px 0", color: "#1E293B" }}>📂 Documents Verified by BMM (4)</h4>
        ...
        {/* Bank Revert Reason Box */}
        {isRevertedByBank && (
          <div style={styles.revertByBankBox}>
            <span style={{ fontWeight: "bold", color: "#9a3412", fontSize: "15px" }}>
                Attention: This application was reverted by the Bank
            </span>
            <p style={{ margin: "5px 0 0 0", color: "#7c2d12", fontSize: "13px" }}>
              <strong>Bank Remarks:</strong> {app.revertReason}
            </p>
          </div>
        )}
        ...
        {isForwarded ? (
          <div style={styles.forwardedBox}>
            <span style={{ fontWeight: "bold", color: "#0a6905", fontSize: "15px" }}>
              ✓ This application has been approved and forwarded to the Bank.
            </span>
          </div>
        ) : isReverted ? (
          <div style={styles.revertBox}>
            <span style={{ fontWeight: "bold", color: "#991b1b", fontSize: "15px" }}>
              ❌ This application was reverted back to the BMMU.
            </span>
          ...
          <div style={styles.revertBox}>
            <label style={{ display: "block", fontWeight: "bold", color: "#991b1b", marginBottom: "8px" }}>
              Reason for Reverting to BMMU:
            </label>
            <textarea
              value={revertComment}
              onChange={(e) => setRevertComment(e.target.value)}
              placeholder="Specify the issue with the documents or details..."
          ...
          <label style={styles.verificationBox}>
            <input 
              ...
              checked={bmmVerified}
              onChange={(e) => setBmmVerified(e.target.checked)}
            />
            <span style={{ fontWeight: "bold", color: "#1e293b", fontSize: "14px" }}>
              All BMM-verified details have been checked. I approve forwarding this to the Bank.
            </span>
          </label>
        )}
          ...
          <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
            ...
            {!isForwarded && !isReverted && (
              isReverting ? (
                ...
                <button 
                  style={bmmVerified ? styles.forwardBtn : styles.disabledBtn} 
                  disabled={!bmmVerified}
...
const styles = {
  ...
  forwardedBox: { backgroundColor: "#dcfce7", padding: "12px", borderRadius: "8px", border: "1px solid #bbf7d0", marginTop: "10px", textAlign: "center" },
  ...
  revertByBankBox: { backgroundColor: "#fffbeb", padding: "12px", borderRadius: "8px", border: "1px solid #fcd34d", marginBottom: "15px" },
