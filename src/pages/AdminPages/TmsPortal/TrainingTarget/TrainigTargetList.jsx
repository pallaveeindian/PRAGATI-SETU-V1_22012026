...
export default function SmmuTargetAchievement() {
  ...
  const [selectedTheme, setSelectedTheme] = useState(""); // Unlocked Theme Filter

  // Dropdown Options State
  const [districts, setDistricts] = useState([]);
  const [plans, setPlans] = useState([]);
  const [themesList, setThemesList] = useState([]); // All Themes List
  ...
  async function fetchTargetsWithAchievements(page = 1) {
    ...
    try {
      const resp = await TMS_API.trainingPartnerTargets.list({
        ...
        partner_name: searchPartner || undefined,
      });
  ...
  // Triggers fetch on page change
  useEffect(() => {
    fetchTargetsWithAchievements(currentPage);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPage]);

  // Initial load - Fetch Districts, All Themes, and All Plans without locking
  useEffect(() => {
    // Fetch Districts
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      ...
                      {themesList.map((th) => (
                        <option key={th.id} value={th.theme_name}>
                          {th.theme_name}
                        </option>
                      ))}
                    </select>
