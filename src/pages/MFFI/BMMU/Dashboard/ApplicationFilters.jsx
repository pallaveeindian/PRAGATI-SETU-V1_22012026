...
export default function ApplicationFilters({
  ...
  const executeFiltering = (currentFilters) => {
    ...
    const result = applications.filter((app) => {
      const matchesBlock =
        ...
        normalizeValue(app.block) === normalizeValue(currentFilters.block);
      const matchesPanchayat =
        !currentFilters.panchayat ||
        normalizeValue(app.panchayat) ===
          normalizeValue(currentFilters.panchayat);
      const matchesVillage =
        !currentFilters.village ||
        normalizeValue(app.village) === normalizeValue(currentFilters.village);
      const matchesSocialCategory =
        !currentFilters.socialCategory ||
        normalizeValue(app.socialCategory) ===
          normalizeValue(currentFilters.socialCategory);
      const matchesReligion =
        !currentFilters.religion ||
        normalizeValue(app.religion) ===
          normalizeValue(currentFilters.religion);

      return (
        matchesBlock &&
        matchesPanchayat &&
        matchesVillage &&
        matchesSocialCategory &&
        matchesReligion
      );
    });
  ...
  useEffect(() => {
    ...
    async function loadUserGeoscope() {
      ...
      try {
        ...
        const geoscope =
          cached || (await LOOKUP_API.userGeoscopeByUserId(user.id))?.data;
        try {
          localStorage.setItem("ps_user_geoscope", JSON.stringify(geoscope));
        } catch (e) {}
        ...
        if (isMounted && isBmmuUser) {
          const geoBlockId =
            geoscope &&
            (geoscope.block_id ||
              (Array.isArray(geoscope.blocks) &&
                (geoscope.blocks[0]?.block_id ||
                  geoscope.blocks[0]?.id ||
                  geoscope.blocks[0])));
          ...
          if (blockId) {
            ...
            if (
              !blockObj ||
              !resolveName(blockObj, ["block_name_en", "block_name", "name"])
            ) {
              try {
                const listRes = await LOOKUP_API.blocks.list({
                  page_size: 1000,
                });
                ...
                const foundBlock = allBlocks.find(
                  (b) =>
                    ...
                    String(b.block_id) === String(blockId) ||
                    String(b.api_block_code) === String(blockId),
                );
            ...
            if (!blockObj)
              blockObj = { id: blockId, block_name_en: String(blockId) };

            const blockName =
              resolveName(blockObj, [
                "block_name_en",
                "block_name",
                "name",
                "blockName",
            ...
            if (isMounted) {
              ...
              const newFilters = {
                ...
                block:
                  blockName !== String(blockId) ? blockName : filters.block,
                panchayatId: "",
                panchayat: "",
                villageId: "",
                village: "",
  ...
  useEffect(() => {
    ...
    async function loadPanchayats() {
      try {
        ...
        if (isMounted) {
          setPanchayats(extractLookupList(res));
          setVillages([]);
        }
      } catch (err) {
        if (isMounted) {
  ...
  const handlePanchayatChange = (e) => {
    setFilters((prev) => ({
      ...
      panchayat: e.target.selectedOptions[0]?.text || "",
      villageId: "",
      village: "",
    }));
  };
  ...
  const handleVillageChange = (e) => {
    setFilters((prev) => ({
      ...
      village: e.target.selectedOptions[0]?.text || "",
    }));
  };

  const visibleBlocks =
    isBmmuUser && isBlockLocked && lockedBlock ? [lockedBlock] : blocks;

  return (
    <div style={styles.filterCard}>
      <h4 style={{ margin: "0 0 10px 0", color: "#6D2A00" }}>
        Filter Applications
      </h4>
      <div style={{ display: "flex", gap: "15px", flexWrap: "wrap" }}>
        {isBmmuUser && isBlockLocked ? (
          <div style={styles.lockedField}>
            ...
            <span style={styles.lockedValue}>
              {(filters.block !== String(filters.blockId)
                ? filters.block
                : null) ||
                lockedBlock?.name ||
                lockedBlock?.block_name_en ||
                "Assigned Block"}
            </span>
          </div>
        ) : (
          ...
          >
            ...
            {visibleBlocks.map((b, i) => {
              const blockId =
                typeof b === "string" ? b : b.id || b.block_id || i;
              const blockName =
                resolveName(b, ["block_name_en", "name", "label"]) ||
                String(blockId);
              return (
        ...
        >
          ...
          {panchayats.map((p, i) => {
            const panchayatId =
              typeof p === "string" ? p : p.id || p.panchayat_id || i;
            const panchayatName =
              resolveName(p, ["panchayat_name_en", "name", "label"]) ||
              String(panchayatId);
            return (
              <option key={panchayatId} value={panchayatId}>
                {panchayatName}
              </option>
            );
          })}
        ...
        >
          ...
          {villages.map((v, i) => {
            const villageId =
              typeof v === "string" ? v : v.id || v.village_id || i;
            const villageName =
              resolveName(v, ["village_name_en", "name", "label"]) ||
              String(villageId);
            return (
        ...
        <select
          ...
          onChange={(e) =>
            setFilters((prev) => ({ ...prev, socialCategory: e.target.value }))
          }
        >
...
const styles = {
  ...
  applyBtn: {
    padding: "8px 15px",
    backgroundColor: "#6D2A00",
    color: "white",
    border: "none",
    borderRadius: "6px",
    cursor: "pointer",
    fontWeight: "bold",
    boxShadow: "0 2px 4px rgba(47, 95, 179, 0.2)",
  },
  resetBtn: {
    padding: "8px 15px",
