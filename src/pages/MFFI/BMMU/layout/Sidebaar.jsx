...
// Yaha FaHome aur FaBuilding import kiya hai
import { 
  FaBars, 
  FaArrowLeft, 
  FaHome, 
...
const Sidebaar = ({
  ...
  const menuItems = ['Dashboard', 'Application List', 'Report Section'];
  const navigate = useNavigate();

  const roleSegment = (currentRole || 'bmmu').toLowerCase();

  const routeFor = (item) => {
    if (item === 'Dashboard') return `/mffi/${roleSegment}/dashboard`;
    if (item === 'Application List') return `/mffi/${roleSegment}/dashboard/applications`;
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <span style={{ fontSize: '18px', width: '24px', textAlign: 'center', display: 'flex', alignItems: 'center' }}>
                  {/* Dashboard ke liye Home icon */}
                  {item === 'Dashboard' && <FaHome />} 
                 
                  
                  {item === 'Application List' && <FaClipboardList />}
