...
export default function AnalyticsSection({ currentReport }) {
  ...
  const fetchReportData = useCallback(async () => {
    ...
    try {
      if (
        ...
        // Fetch UP at a glance (Used for global overview AND TMS Demographics)
  ...
  useEffect(() => {
    fetchReportData();
  }, [activeTab, activeSubTab]); // Removed filters from dependencies to only fetch on "Apply" button if desired, but we can auto-fetch or fetch on apply. Let's rely on the child's "Apply" button.
