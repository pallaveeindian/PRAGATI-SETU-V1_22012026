...
import apiPayloadCSV from "../../../assets/abapi_data/api_payload.csv?raw";
import blocksListCSV from "../../../assets/abapi_data/blocks_list.csv?raw";
import dataPeriodCSV from "../../../assets/abapi_data/dataperiod.csv?raw";
import deptListCSV from "../../../assets/abapi_data/dept_list.csv?raw";
import districtsListCSV from "../../../assets/abapi_data/districts_list.csv?raw";
import indicatorCodesCSV from "../../../assets/abapi_data/indicatorcodes_list.csv?raw";
import periodicityCSV from "../../../assets/abapi_data/periodicity.csv?raw";
...
export const getBlockByLokosId = (lokosBlockId) => {
  const block = store.blocks.find(
    (b) => String(b.lokos_block_id) === String(lokosBlockId),
  );
...
export const getDistrictByLokosId = (lokosDistrictId) => {
  const district = store.districts.find(
    (d) => String(d.lokos_district_id) === String(lokosDistrictId),
  );
...
export const getIndicatorDetails = (indicatorCode) => {
  const indicator = store.indicators.find(
    (i) => String(i.IndicatorCode) === String(indicatorCode),
  );
...
export const getDepartmentDetails = (deptCode) => {
  const dept = store.departments.find(
    (d) => String(d.api_dept_code) === String(deptCode),
  );
  return dept
    ? {
...
export const getMonthDetails = (monthCode) => {
  const month = store.months.find(
    (m) => String(m.api_month_code) === String(monthCode),
  );
  return month
    ? {
...
export const getPeriodicityDetails = (periodCode) => {
  const period = store.periods.find(
    (p) => String(p.api_period_code) === String(periodCode),
  );
  return period
    ? {
...
export const getDisclaimerTemplate = () => {
  ...
  const row = store.payloadTemplate.find(
    (r) => r["Column Name"] === "Disclaimer",
  );
