...
export default function SmmuTargetAchievement() {
  ...
  const processedData = useMemo(() => {
    return targetsData.map((row) => {
      ...
      const achievedCount = row.achievements?.reduce(
      ...
      const progressPct = targetCount > 0 ? Math.round((achievedCount / targetCount) * 100) : 0;
      ...
      <div className="main-area">
        ...
        >
          ...
          >
            ...
            >
              <h4 style={{ margin: "0 0 12px 0", color: "#2b4e72" }}>Filters & Export</h4>
              <form 
                onSubmit={fetchTargetsWithAchievements} 
                style={{ display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "flex-end" }}
              >
                <div>
                  <label style={{ display: "block", fontSize: "13px", fontWeight: "600", color: "#2b4e72", marginBottom: "4px" }}>
                    Financial Year
                  </label>
                  <select
                    ...
                    style={{ width: "160px", padding: "8px", borderRadius: "6px", border: "1px solid #a7c6ed" }}
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    style={{ width: "220px", padding: "8px", borderRadius: "6px", border: "1px solid #a7c6ed" }}
                  />
                </div>
            ...
            >
              <h2 style={{ margin: 0, color: "#2b4e72" }}>Training Partners - Targets vs Achievement</h2>
            </div>
            ...
            >
              ...
              >
                <table className="training-table">
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={6} style={{ textAlign: "center", padding: "20px" }}>No targets found.</td>
                      </tr>
                    ) : (
                      paginatedData.map((r) => (
                        <tr key={r.id}>
                          <td style={{ fontWeight: "600", color: "#1e293b" }}>{r.partnerName}</td>
                          <td>
                            <div style={{ fontSize: "14px", color: "#0f172a" }}>{r.planName}</div>
                            <div style={{ fontSize: "12px", color: "#64748b" }}>Theme: {r.theme || "—"}</div>
                          </td>
                          <td>{r.districtName}</td>
                          <td style={{ fontWeight: "bold", color: "#3d6ba6" }}>{r.targetCount}</td>
                          <td style={{ fontWeight: "bold", color: "#10b981" }}>{r.achievedCount}</td>
                          <td>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                              <span style={{ 
                                  ...
                                  padding: "3px 8px", 
                                  borderRadius: "99px",
                                  background: r.progressPct >= 100 ? "#dcfce7" : r.progressPct > 0 ? "#fef3c7" : "#f1f5f9",
                                  color: r.progressPct >= 100 ? "#166534" : r.progressPct > 0 ? "#92400e" : "#475569"
                                }}>
                                {r.progressPct}%
                              </span>
                            </div>
                ...
                <div className="mobile-card-list">
                  {loading ? (
                    <div className="mobile-card" style={{ textAlign: "center" }}>Loading...</div>
                  ) : filteredData.length === 0 ? (
                    <div className="mobile-card" style={{ textAlign: "center" }}>No targets found</div>
                  ) : (
                    paginatedData.map((r) => (
                      <div key={r.id} className="mobile-card">
                        ...
                        <div><strong>Plan:</strong> {r.planName}</div>
                        <div><strong>District:</strong> {r.districtName}</div>
                        <div style={{ display: "flex", justifyContent: "space-between", marginTop: "8px", paddingTop: "8px", borderTop: "1px solid #e2e8f0" }}>
                          <div><strong>Target:</strong> {r.targetCount}</div>
                          <div><strong>Achieved:</strong> {r.achievedCount}</div>
                          <div><strong>Progress:</strong> {r.progressPct}%</div>
                        </div>
                      </div>
                    ))
                  )}
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      {[...Array(totalPages)].map((_, i) => {
                        ...
                        if (pageNum === currentPage - 2 || pageNum === currentPage + 2) {
                           return <span key={pageNum} style={{ alignSelf: "center", color: "#64748b" }}>...</span>;
                        }
                        return null;
                      })}
