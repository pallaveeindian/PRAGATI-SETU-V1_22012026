...
export default function AdminDashKPI({ data }) {
  ...
  const themes = data.theme_wise_performance || [];
  ...
  const {
    ...
    pending_pax_count = 0,
    scheduled_batches = 0,
    scheduled_pax_count = 0,
    ongoing_batches = 0,
    ongoing_pax_count = 0,
    completed_batches = 0,
    completed_pax_count = 0,
    review_batches = 0,
    review_pax_count = 0,
    closed_batches = 0,
    closed_pax_count = 0,
    rejected_batches = 0,
    rejected_pax_count = 0,
  ...
  const cards = [
    {
      ...
      iconBg: "#e0e7ff",
      color: "#3b82f6",
      trendText: total_pax_count,
      trendSuffix: "Participants",
      status: "ALL",
    },
    {
    ...
    },
    {
      ...
      color: "#a855f7",
      trendText: "Onboarded",
      trendSuffix: "across Training Requests",
    },
    {
  ...
  return (
    <div className="admin-kpi-wrapper">
      {/* HIGHLIGHTED: OVERALL TARGETS VS ONBOARDED */}
      <div className="target-highlight-banner">
        <div className="tb-left">
          <h3 className="tb-title">
            Total Participants Onboarded vs Overall Target
          </h3>
          <div className="tb-numbers">
            <span className="tb-onboarded">
              {totalOnboarded.toLocaleString()}
            </span>
            <span className="tb-divider">/</span>
            <span className="tb-target">{totalTarget.toLocaleString()}</span>
          </div>
        </div>
        <div className="tb-right">
          ...
          <div className="tb-progress-bar-bg">
            <div
              className="tb-progress-bar-fill"
              style={{ width: `${Math.min(achievementPercentage, 100)}%` }}
            ></div>
          </div>
          <div className="tb-subtitle">
            Remaining:{" "}
            {Math.max(0, totalTarget - totalOnboarded).toLocaleString()} to
            onboard
          </div>
        </div>
      </div>
